  ...
  const login = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/api-key-not-valid') {
        alert("Erro de Configuração: O Firebase não está configurado corretamente. Por favor, execute o 'Set up Firebase' no menu do AI Studio.");
  ...
  if (!user) {
    return (
      <button 
        ...
        className={`neu-convex bg-base p-2 rounded-full text-slate-600 hover:text-blue-500 transition-all ${isSigningIn ? 'opacity-50 cursor-wait animate-pulse' : ''}`}
